...
from smooth.algo import smooth_mechanism_tvec_secure, thres_smooth_vec
...
filelog = None
folder_prefix = './results/test_thres_<thres>_<eps>_calihh<jrep>_secure'
jrep = 0#1#2#3#4
mreps = 100
...
pg3 = 1.0/3.0
mech_base0 = lambda arr_x, n: [func_tau_post(GP_loc_vec_secure(x, eps, mreps=mreps)) for x in arr_x[n]]
mech_base2 = lambda arr_x, n: GP_post_func_vec_secure(arr_x[n], eps, GP_func2, sens2, post_func2, mreps)
mech_smootht3 = lambda arr_x, n: thres_smooth_vec(arr_x[n], eps, thres, tau, mreps=mreps, smooth_mech_vec=smooth_mechanism_tvec_secure,nu=3,pg=pg3)
mech_lap = lambda arr_x, n: ldp_lap_vec_secure(arr_x[n], GP_func2, epsL, mreps, sensL)
