...
from baseline.algo import GP_loc_vec, GP_post_func_vec_secure
from ldp.algo import ldp_lap_vec_secure
from smooth.algo import smooth_mechanism_tvec_secure, twthres_smooth_vec
...
filelog = None
folder_prefix = './results/twgrid_secure/test_twthresh_<T1>_<eps>_calihhincdebt<trange>'
mreps = 50
gcell = 33
...
mech_base2 = lambda arr_x, T: GP_post_func_vec_secure(arr_x, eps, (lambda x: GP_func2(x,T)), sens_func2(T), post_func2, mreps)
mech_smootht3 = lambda arr_x, T: twthres_smooth_vec(arr_x, eps, T1=T[0], T2=T[1], tau=dict_taus[str(T[0])+'_'+str(T[1])], mreps=mreps, smooth_mech_vec=smooth_mechanism_tvec_secure,nu=3,pg=pg3)
